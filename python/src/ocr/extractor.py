...
class TableExtractor:
    ...
    def extract_table(self, image: Image.Image, lang: str = 'eng') -> Dict[str, Any]:
        """
        Extract table from image crop with structure preservation
        ...
        Returns:
            Dictionary with 'text' (tab-separated CSV) and metadata
        """
        ...
        # Group into rows (vertical clustering)
        ...
        # Detect columns (horizontal clustering)
        all_x = set()
        for row in rows:
            for word in row:
                all_x.add(word['left'])
        ...
        for row_words in rows:
            ...
            # Check if row has actual alphanumeric content
            # Extract only alphanumeric characters (letters and digits)
            import re
            alphanumeric_only = re.sub(r'[^a-zA-Z0-9]', '', row_text)

            # Only add row if it has at least 2 alphanumeric characters
            # This filters out rows with only separators (—, -, =, etc.)
