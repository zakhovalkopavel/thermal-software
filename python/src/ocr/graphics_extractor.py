...
class GraphicsExtractor:
    ...
    def __init__(self, graphics_dir: Path = None, min_size: int = 100, edge_density_threshold: float = 0.1, min_size_percent: float = None):
        """
        ...
        Args:
            ...
            min_size_percent: Minimum size as percentage of page (e.g., 0.1 for 10%)
        """
        self.graphics_dir = Path(graphics_dir) if graphics_dir else Path('/tmp/graphics')
    ...
    def detect_graphics(self, image: Image.Image, min_size_percent: float = None) -> List[Dict]:
        """
        Detect graphics regions without saving

        Args:
            image: PIL Image to process
            min_size_percent: Minimum size as percentage of page (overrides init value)

        Returns:
            List of regions with bbox coordinates
        """
        img_array = np.array(image.convert('RGB'))
        gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)

        page_h, page_w = gray.shape
        min_percent = min_size_percent or self.min_size_percent or 0.1
        min_graphic_w = int(page_w * min_percent)
        min_graphic_h = int(page_h * min_percent)
    ...
    def save_graphic(self, image: Image.Image, page_num: int, index: int, source_name: str) -> Path:
        """
        Save a single graphic image

        Args:
            image: PIL Image to save
            page_num: Page number
            index: Graphic index on page
            source_name: Source document name

        Returns:
            Path to saved file
        """
        filename = f"{source_name}_page{page_num:03d}_graphic{index:03d}.png"
