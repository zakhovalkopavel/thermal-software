...
class LayoutAnalyzer:
    ...
    def analyze(self, image: Image.Image) -> Dict[str, Any]:
        """
        Analyze page layout and detect text blocks

        Args:
            image: PIL Image to analyze

        Returns:
            Dictionary with 'text_blocks' and other layout info
        """
        # Convert to array
        ...
        for word in words_data:
            if word['block_num'] != last_block_num:
                # Save previous block
                if current_block is not None:
                    ...
                    if block_w > 150 and block_h > 60 and (block_w / block_h) > 1.5:
                        text_blocks.append({
                ...
                last_block_num = word['block_num']
            else:
                # Expand block
                current_block[0] = min(current_block[0], word['left'])
                current_block[1] = min(current_block[1], word['top'])
                current_block[2] = max(current_block[2], word['left'] + word['width'])
                current_block[3] = max(current_block[3], word['top'] + word['height'])
