...
Input composition (wt%)
         ...
         ▼
SiO₂ > 50 wt%?
    ├─ NO ──► Neither model valid. Report ERROR_NO_VALID_MODEL.
    └─ YES
         │
         ▼
All non-zero components in {SiO₂, Al₂O₃, Na₂O, K₂O, Li₂O, CaO, MgO, BaO, ZnO, PbO, B₂O₃}?
    ├─ YES (or only trace amounts outside) ──► Lakatos eligible
    └─ NO ──► Lakatos not eligible (or eligible with WARNING_COMPONENTS_IGNORED)
         │
         ▼
All components within Fluegel_table3.csv bounds?
    ├─ YES ──► Fluegel eligible
    └─ NO (some out of range) ──► Fluegel eligible with CONFIDENCE_REDUCED
         │
         ▼
Both eligible? ──► DUAL MODE (both models, compare)
Only Lakatos? ──► LAKATOS ONLY
Only Fluegel? ──► FLUEGEL ONLY
Neither? ──► ERROR_NO_VALID_MODEL
```
